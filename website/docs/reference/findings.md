...
| `view_filter_exceeds_editor_capacity` | warning | A view's `where` renders more than 10 comparisons, which is more than the classic filter editor can show. The emitted filter is protected, so this is survivable; if that protection is ever removed or fails to take, the view loses the comparisons past the ceiling the first time somebody opens its settings. A negated clause renders two comparisons, not one. |
