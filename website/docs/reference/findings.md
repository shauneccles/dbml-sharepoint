...
| `view_filter_exceeds_editor_capacity` | warning | A view's `where` renders more than 10 conditions, which is more than the classic filter editor can show. The emitted filter is protected so an operator cannot truncate it by pressing Save, and the view can no longer be adjusted in the UI at all. |
