...
(async () => {
  ...
  // ---- Did the guard actually take? -------------------------------------
  // Every filtered view is emitted in a shape the filter editor refuses to
  // open, because an editor that opens a filter writes back only the ten
  // conditions it rendered (#267). The readback above already proves the
  // stored ViewQuery is the declared one, per view. What it cannot see is
  // whether THIS tenant's editor still refuses that shape, which is a
  // property of SharePoint's UI on the day of the deploy.
  //
  // One fetch, not one per view: the guard is identical across views and
  // each view's stored query is already verified, so one page answers for
  // all of them. The page measured 501,773 characters on 2026-08-17, so
  // per-view would add roughly 96MB to a 192-view bundle.
  //
  // Measured 2026-08-17 by test/manual/view-edit-page-probe.js. C1 and C2:
  // the editor's own control names are on an editable page and on an
  // unfiltered one, and absent from a refused one. Control NAMES rather than
  // the English refusal message, which would misreport a non-English tenant.
  const EDITOR_CONTROL = 'FieldPicker1';
  // C6: a request for a view that does not exist answers HTTP 200 on this
  // URL with no editor controls, so absence alone would call a page that is
  // not a view "protected". `ctl00` and `ViewEdit` are on that page too and
  // are useless here; only this one tracks the filter section.
  ...
  async function confirmEditorRefusesTheGuard() {
    ...
    if (body.includes(EDITOR_CONTROL)) {
      // The page arrived and the editor is on it, so the filter is editable
      // and an operator can truncate it. That is a determination, not an
      // absence of one, so it fails the run rather than warning.
      const message = `view '${guarded.title}' on '${guarded.list}' is still editable in the filter`
        + ' editor, so its filter can be truncated by an operator pressing Save';
